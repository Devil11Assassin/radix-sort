...
private:
	...
	static void sortNonRecursiveM(std::vector<std::string>& v, std::vector<std::string>& tmp, std::vector<Region>& regions, std::unique_lock<std::mutex>& lkRegions, Region initialRegion, bool multiThreaded);
